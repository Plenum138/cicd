...
// console.log(dateTime)
  ...
  var jobs = schedule.scheduleJob(Schedule_id, dateTime, async (fireDate) => {
      ...
      const getClassStudent = await UserLogins.find({_id : {$nin  : item.remove_user_id} ,school_id: item.school_id, roles: 'STUDENT', class_name: item.class, stream_id: item.stream_id, year: item.year, 'firebase_token': { $ne: null } }).lean().exec();
      console.log( "\u001b[1;32m " ,'fireDate',fireDate );
    ...
    //   console.log('item' ,item.remove_user_id)
      ...
      if (getClassStudent.length > 0) {
        ...
        //Send Push NotificationS
          await sendNotificationClass("207", data.title, data.message, data.tokenArray , data.type);
