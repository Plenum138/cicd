...
class RoleList extends React.Component {
  constructor(props) {
    ...
    setTimeout(() => document.title = 'Role List', 100);
  }
  componentDidMount() {
    this.ListFun();
    this.props.dispatch({ type: 'roles/clearAction' });
  }
...
if (user.role === "ADMIN") {
  ...
  this.props.dispatch({ type: 'roles/roleList', payload: searchval, });
} else {
  let search = 'page=' + (this.state.current - 1) + "&limit=" + this.state.limit + "&inactive=" + this.state.inactive + "&searchText=" + this.state.searchText + "&sortBy=" + this.state.sortBy;
  localStorage.setItem('newsSearch', JSON.stringify(this.state))
...
paginationFun = (val) => this.setState({ current: val.current }, () => this.ListFun());
  ...
  searchVal = (val) => {
    ...
    const resultAutos = this.props.roles.list.data.filter((auto) => auto.roles.toLowerCase().includes(val.toLowerCase()))
  ...
  deleteCat = id => {
		this.props.dispatch({ type: 'roles/roleDelete', payload: { _id: id }, });
  }
  
  getSnapshotBeforeUpdate(prevProps, prevState) {

		if (this.props.roles.del && this.props.roles.del.count > this.state.count && this.props.roles.del.status) {
			this.setState({ count: this.props.roles.del.count })
			return true
		}
		if (this.props.roles.add && this.props.roles.add.count > this.state.Addcount && this.props.roles.add.status) {
			this.setState({ Addcount: this.props.roles.add.count, addModel: false })
			return true
		}
		return null;
	}
  ...
  render() {
    ...
    if (this.state.searchText == '') {
      this.state.listData = roles.list ? roles.list.data : [];
    }
    ...
    const productsAvilabilityColumns = [
      {
        title: <strong>EMPLOYEE ROLE</strong>,
        dataIndex: 'roles',
        render:(val,data) => {
          return (data.roles? <span className="text-info cursor-pointer	" >{data.roles}</span>:'-')
        }
      },
      {
      ...
      },
      {
        ...
        render: (val, data) => <div onClick={e => e.stopPropagation()}>
          <Popconfirm title="Are you sure delete roles?" onConfirm={e => { this.deleteCat(data._id); e.stopPropagation() }} okText="Yes" cancelText="No" >
      ...
      <Card>
        ...
        <Table
          ...
          onRow={(record, rowIndex) => {
            return {
              onClick: roles => this.setState({ addModel: true, detail: record })
            };
          }}
          pagination={{
								position: ['bottomLeft'],
...
export default connect(({ roles, loading }) => ({
  roles, 
  loading
}))(RoleList);
