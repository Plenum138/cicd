...
class ClassList extends React.Component {
  constructor(props) {
    ...
    setTimeout(() => document.title = 'Class List', 100);
  }
  componentDidMount() {
    this.ListFun();
    this.props.dispatch({ type: 'classes/clearAction' });
  }
...
if (user.role === "ADMIN") {
  ...
  this.props.dispatch({ type: 'classes/classList', payload: searchval, });
} else {
  let search = 'page=' + (this.state.current - 1) + "&limit=" + this.state.limit + "&inactive=" + this.state.inactive + "&searchText=" + this.state.searchText + "&sortBy=" + this.state.sortBy;
  localStorage.setItem('newsSearch', JSON.stringify(this.state))
...
paginationFun = (val) => this.setState({ current: val.current }, () => this.ListFun());
  ...
  searchVal = (val) => {
    ...
    const resultAutos = this.props.classes.list.data.filter((auto) => auto.year.toLowerCase().includes(val.toLowerCase()) || auto.class_name.toLowerCase().includes(val.toLowerCase()) || auto.stream_name.toLowerCase().includes(val.toLowerCase()))
  ...
  deleteCat = id => {
		this.props.dispatch({ type: 'classes/classDelete', payload: { _id: id }, });
  }
  
  getSnapshotBeforeUpdate(prevProps, prevState) {

		if (this.props.classes.del && this.props.classes.del.count > this.state.count && this.props.classes.del.status) {
			this.setState({ count: this.props.classes.del.count })
			return true
		}
		if (this.props.classes.add && this.props.classes.add.count > this.state.Addcount && this.props.classes.add.status) {
			this.setState({ Addcount: this.props.classes.add.count, addModel: false })
			return true
		}
		return null;
	}
  ...
  render() {
    ...
    if (this.state.searchText == '') {
      this.state.listData = classes.list ? classes.list.data : [];
    }
    ...
    const productsAvilabilityColumns = [
      {
        title: <strong>Stream Name</strong>,
        dataIndex: 'stream_name',
        render:(val,data) => {
          return (data.stream_name? <span className="text-info cursor-pointer	" >{data.stream_name}</span>:'-')
        }
      },
      {
        title: <strong>Year Name</strong>,
        dataIndex: 'year',
       
      },  {
        title: <strong>Class Name</strong>,
      ...
      },
      {
        ...
        render: (val, data) => <div onClick={e => e.stopPropagation()}>
          <Popconfirm title="Are you sure delete classes?" onConfirm={e => { this.deleteCat(data._id); e.stopPropagation() }} okText="Yes" cancelText="No" >
      ...
      <Card>
        ...
        <Table
          ...
          onRow={(record, rowIndex) => {
            return {
              onClick: classes => this.setState({ addModel: true, detail: record })
            };
          }}
          pagination={{
								position: ['bottomLeft'],
...
export default connect(({ classes, loading }) => ({
  classes, 
  loading
}))(ClassList);
